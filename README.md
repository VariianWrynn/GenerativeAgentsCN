...
- `agents_file` - (可选) 指定一个JSON文件路径，用于加载自定义的智能体名称列表。如果未提供此参数，则使用默认的智能体列表。JSON文件应包含一个字符串列表，其中每个字符串都是一个智能体名称。例如：
  ```json
  [
    "阿伊莎",
    "克劳斯",
  ...
  ```
  注意：文件中指定的智能体名称必须存在于`generative_agents/start.py`中预定义的`personas`列表中才能生效。无效或不存在的名称将被忽略，并会打印警告信息。如果文件无法找到、格式错误或过滤后没有有效的智能体，脚本将报错并退出。
